...
"""
 test07
 - test sensitivity of dynamic coefficients to uv_src_blend coefficient
 - correct accounting of uv_src_blend in zeroth and first-order problems
   results in invariant dynamic coefficients
...
def main():
    ...
    blends = np.array([0.25, 0.5, 0.75, 1.0])
    ...
    # arrays for dynamic coefficients
    K = np.zeros(blends.size, dtype=np.float64)
    k = np.zeros(blends.size, dtype=np.float64)
    D = np.zeros(blends.size, dtype=np.float64)
    d = np.zeros(blends.size, dtype=np.float64)
    M = np.zeros(blends.size, dtype=np.float64)
    ...
    for idx_blend, blend in enumerate(blends):
        s.uv_src_blend = blend
        s.solve_zeroth()
        ...
        K[idx_blend] = px_r[0]/1.e6
        k[idx_blend] = py_r[0]/1.e6
        D[idx_blend] = px_i[0]/1.e3
        d[idx_blend] = py_i[0]/1.e3
        M[idx_blend] = px_r[1]
    ...
    plt.figure()
    #plt.scatter(pertFreq, np.imag(fx)/1.e6, label='fx-data')
    plt.plot(blends, K, label = r'$K_{xx}$')
    plt.plot(blends, k, label = r'$K_{yx}$')
    #plt.scatter(pertFreq, np.imag(fy)/1.e6, label='fy-data')
    #plt.plot(pertFreq, linear_func(pertFreq, *py_i)/1.e6, 'r--', label='fit: a=%5.3f' % tuple(py_i))
    plt.xlabel(r'uv_src_blend')
    plt.ylabel(r'Stiffness [MN/m]')
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig('K_vs_blend.png')
    plt.close()
    
    plt.figure()
    #plt.scatter(pertFreq, np.imag(fx)/1.e6, label='fx-data')
    plt.plot(blends, D, label = r'$D_{xx}$')
    plt.plot(blends, d, label = r'$D_{yx}$')
    #plt.scatter(pertFreq, np.imag(fy)/1.e6, label='fy-data')
    #plt.plot(pertFreq, linear_func(pertFreq, *py_i)/1.e6, 'r--', label='fit: a=%5.3f' % tuple(py_i))
    plt.xlabel(r'uv_src_blend')
    plt.ylabel(r'Damping [KN.s/m]')
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig('D_vs_blend.png')
    plt.close()
