...
"""
 test08
 - test sensitivity of dynamic coefficients to relax_uv momentum relaxation
   coefficient
 - 
...
def main():
    ...
    relaxes = np.array([0.1, 0.25, 0.5, 0.75, 0.8])
    ...
    K = np.zeros(relaxes.size, dtype=np.float64)
    k = np.zeros(relaxes.size, dtype=np.float64)
    D = np.zeros(relaxes.size, dtype=np.float64)
    d = np.zeros(relaxes.size, dtype=np.float64)
    M = np.zeros(relaxes.size, dtype=np.float64)
    ...
    for idx_relax, relax in enumerate(relaxes):
        s.relax_uv = relax
        s.relax_uv1 = relax
        s.solve_zeroth()
        ...
        K[idx_relax] = px_r[0]/1.e6
        k[idx_relax] = py_r[0]/1.e6
        D[idx_relax] = px_i[0]/1.e3
        d[idx_relax] = py_i[0]/1.e3
        M[idx_relax] = px_r[1]
    ...
    plt.figure()
    #plt.scatter(pertFreq, np.imag(fx)/1.e6, label='fx-data')
    plt.plot(relaxes, K, label = r'$K_{xx}$')
    plt.plot(relaxes, k, label = r'$K_{yx}$')
    #plt.scatter(pertFreq, np.imag(fy)/1.e6, label='fy-data')
    #plt.plot(pertFreq, linear_func(pertFreq, *py_i)/1.e6, 'r--', label='fit: a=%5.3f' % tuple(py_i))
    plt.xlabel(r'relax_uv')
    plt.ylabel(r'Stiffness [MN/m]')
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig('K_vs_relax.png')
    plt.close()
    
    plt.figure()
    #plt.scatter(pertFreq, np.imag(fx)/1.e6, label='fx-data')
    plt.plot(relaxes, D, label = r'$D_{xx}$')
    plt.plot(relaxes, d, label = r'$D_{yx}$')
    #plt.scatter(pertFreq, np.imag(fy)/1.e6, label='fy-data')
    #plt.plot(pertFreq, linear_func(pertFreq, *py_i)/1.e6, 'r--', label='fit: a=%5.3f' % tuple(py_i))
    plt.xlabel(r'relax_uv')
