...
def main():
    ...
    px_r,cov = optimize.curve_fit(quad_func, pertFreq, np.real(fx[0,:]), p0=[0.0, 0.0]) 
    py_r,cov = optimize.curve_fit(quad_func, pertFreq, np.real(fy[0,:]), p0=[0.0, 0.0])     
    px_i,cov = optimize.curve_fit(linear_func, pertFreq, np.imag(fx[0,:]), p0=[0.0]) 
    py_i,cov = optimize.curve_fit(linear_func, pertFreq, np.imag(fy[0,:]), p0=[0.0]) 
